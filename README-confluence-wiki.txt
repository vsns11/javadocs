...
# TTD calls TMF-701 over HTTP to create a processFlow.
# TMF-701 publishes a {{processFlow.created}} event to the {{notification-management}} topic.
# pamconsumer reads {{notification-management}} (both TMF events and external async responses) and publishes {{processFlow.initiated}} and {{task.signal}} messages to the {{task.command}} topic.
# task-orchestrator consumes from {{task.command}}, resolves the DAG, manages the barrier lifecycle, publishes {{task.execute}} commands, and PATCHes the parent processFlow on TMF-701.
# task-runner consumes {{task.execute}} and {{task.signal}} messages, runs its action pipeline (POST taskFlow, downstream call, PATCH taskFlow), and publishes {{task.event}} messages back to {{task.command}}.
...
h3. Envelope fields (always present)
...
h2. Per-Message-Name Field Matrix
...
The pending count is computed in code as task_total minus task_completed minus task_failed.
...
h2. Barrier Lifecycle and Lazy Seeding
...
*# Set status CLOSED, closed_at to now().
*# Look up the next batch in the DAG by batch_index plus 1.
*# If a next batch exists, insert a new barrier row for it with status OPEN, then publish task.execute for each of its actions.
*# If no next batch exists, PATCH the parent processFlow with state completed on TMF-701.
...
h2. Concurrency, Ordering, Idempotency

h3. Layered protection
...
h3. Required Kafka client config (orchestrator)
...
h2. End-to-End Example: SYNC Action

A SYNC action's runner pipeline:

# POST /processFlow/{id}/taskFlow, which mints task.id and task.href.
# Calls the downstream system, blocking.
# PATCH /processFlow/{id}/taskFlow/{taskId} with the result.
# Publishes a single task.event with status COMPLETED and the full result payload.
...
h3. Sample messages for a SYNC action a2 (INTERNET_CHECK)

h4. task.execute (orchestrator to task.command)
...
h4. task.event (task-runner to task.command)
...
#* Stops.
# External system completes the work and publishes a final response on notification-management.
# pamconsumer receives that external event, looks up waiting_task by the business transaction id, and publishes a task.signal on task.command.
# Second invocation, triggered by task.signal:
...
h3. Sample messages for an ASYNC action a1 (VOICE_SERVICE_DIAGNOSTIC)
...
h4. task.event WAITING (task-runner to task.command)
...
h4. task.signal (pamconsumer to task.command)
...
h4. task.event COMPLETED (task-runner to task.command)
...
h3. Orchestrator effect across the ASYNC lifecycle
...
h2. TMF-701 PATCH Contract

h3. Per task: register taskFlow ref on parent processFlow
...
h3. When all batches close: mark processFlow completed
...
h2. Orchestrator Dispatch Table
